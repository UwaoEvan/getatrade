  ...
  return (
    ...
    >
      <div className="max-w-4xl mx-auto p-6">
        <h1 className="text-3xl font-bold mb-4">{shortlisted?.job?.title}</h1>

        <div className="flex items-center text-gray-500 space-x-4 mb-6">
          <span>
            🕒{" "}
            {formatDistanceToNow(new Date(shortlisted?.job?.createdAt || ""), {
              addSuffix: true,
            })}
          </span>
          <span>📍 {shortlisted?.job?.location}</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          {shortlisted?.paid ? (
            <HiredInfo
              name={shortlisted?.user.username}
              email={shortlisted?.user.email}
              phoneNumber={shortlisted?.user?.phoneNumber || undefined}
              jobId={shortlisted?.job.id}
              targetUserId={shortlisted?.job.userId}
              currentUserId={shortlisted?.userId}
              title={shortlisted?.job.title}
              createdAt={shortlisted?.job.createdAt}
              location={shortlisted?.job.location}
            />
          ) : (
            <ShortlistFee
              jobId={jobId}
        ...
        <div className="p-4 rounded-lg mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-700">
            <div>
              <strong>Job type:</strong> {shortlisted?.job.title}
            </div>
            <div>
              <strong>Location:</strong> {shortlisted?.job.location}
            </div>
            <div>
              <strong>Service:</strong> {shortlisted?.job.category}
            </div>
